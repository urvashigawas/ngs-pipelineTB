...
```bash
git clone https://github.com/<your-username>/ngs-pipelineTB.git
cd ngs-pipelineTB
```
...
```
results/
├── fastqc/            # Raw read QC reports (HTML + ZIP)
├── alignment/          # Sorted, indexed BAM files
├── variants/            # Raw and filtered VCF files
└── multiqc/              # Aggregated QC report (planned)
```
...
For testing and validation, this pipeline pairs well with public clinical *M. tuberculosis* WGS data, for example:

- **SRA run:** ERR3275683 (BioProject PRJEB32037)
- **Platform:** Illumina HiSeq 2500, single-end, ~100× coverage over the 4.4 Mb genome
- **Context:** part of a study evaluating WGS-based prediction of first-line drug susceptibility, with phenotypic drug susceptibility testing (DST) results available for comparison

### Genes of clinical interest

| Gene | Drug | Example mutation | Effect |
|---|---|---|---|
| `rpoB` | Rifampicin | S450L | High-confidence resistance |
| `katG` | Isoniazid | S315T | Major resistance mutation |
| `inhA` promoter | Isoniazid | promoter SNPs | Low-level resistance |
| `embB` | Ethambutol | M306V/I/L | Resistance |
| `pncA` | Pyrazinamide | diverse | Resistance |
...
- Align >95% of reads to H37Rv
- Reach the expected coverage for the input dataset
- Produce a small, biologically plausible set of SNPs relative to H37Rv
- Recover known resistance-associated variants when present in the sample
